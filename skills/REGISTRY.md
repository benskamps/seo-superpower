...
and routes to the rest. The other 17 are listed by where they fall in a site's lifecycle.

> 18 skills total. "Triggers on" is a short read of each skill's `description`; the full trigger
...
| [generating-hreflang](generating-hreflang/SKILL.md) | `generating-hreflang` | "hreflang", "international SEO", "multilingual", "x-default", "localize SEO", "alternate language links" | Cross-cutting / Initial → Growth | Syntax validation, bidirectional reciprocal link verification, x-default enforcement, and HTML/XML/Next.js code generation via `scripts/hreflang-tool.js`. |
...
| [mirroring-competitor-codebases](mirroring-competitor-codebases/SKILL.md) | `mirroring-competitor-codebases` | "mirror their codebase", "what is their site doing that mine isn't", "reverse-engineer their SEO setup", "competitor schema/heading/link audit" | Growth → Mature | The Competitor Codebase Mirror: parses the HTML both sites serve, folds each side into per-page rates (schema types, question-heading shape, internal-link density, meta coverage, server-rendered vs client shell), and reports only the patterns they ship and you don't, ranked by SERP delta. Deterministic engine: `scripts/codebase-mirror.js`. |
