...
5. **Validate.** Run the offline schema validator `scripts/schema-check.js` (`node scripts/schema-check.js --file <path>`) or `scripts/schema-quick.py` (`python scripts/schema-quick.py --file <path>`). This instantly checks JSON-LD syntax, catches schema.org casing errors (like `Website` vs `WebSite`), and validates Google Rich Result required/recommended fields without browser switching or external pip dependencies. If the `schema-validate` MCP is installed, it can also be called. Use external tools ([Schema Markup Validator](https://validator.schema.org/) and [Google Rich Results Test](https://search.google.com/test/rich-results)) as secondary browser confirmation [9].
